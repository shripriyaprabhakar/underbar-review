(function() {
  ...
  _.last = function(array, n) {
    return n === undefined ? array[array.length - 1] 
      : array.slice(array.length - Math.min(n, array.length));
  };
  ...
  _.uniq = function(array, isSorted, iterator) {
    ...
    for (var i = 0; i < array.length; i++){
      if (!seen.hasOwnProperty(iterator(array[i]).toString())) {
  ...
  _.shuffle = function(array) {
    ...
    for (var i = array.length; i > 1; i--) {
      var destination = Math.floor(Math.random() * i);
      var temp = result[i - 1];
      result[i - 1] = result[destination];
